...
const getSVGSize = (tree, fontSize=10) => {
...
module.exports = function BuildKPIKarta(treeData, treeContainerDom, options) {
    ...
    getSVGSize(treeData, options?.styleOptions?.fontSize);
  ...
  function update(
    ...
    fontSize = options.styleOptions?.fontSize || 10
        ...
        const dynamicHeight = (Number(fontSize) * 20) / 10;
        var xScale = d3.scale.linear().domain(xExtent).range([dynamicHeight, height - dynamicHeight]); // compress or expand to fit
